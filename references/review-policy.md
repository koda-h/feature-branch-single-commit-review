...
1. `a9e265be` -> `e5837e7a`
   - [修正済み][middle] `path/to/file.ext:123`
     問題点の要約。どう壊れるか、なぜ不足かを簡潔に書く。
     変更前 / 変更後:
     ```diff
     - old_value = load(path)
     + old_value = load(path, strict=True)
     ```
     最新コミットでの状態: 修正済み
...
- 各指摘は文頭の状態ラベルから始める。形式は `[未修正][high] ...` / `[修正済み][middle] ...` / `[要確認][low] ...` のようにする
- 各指摘には `変更前 / 変更後` が読み取れる差分抜粋を必ず付ける。`diff` コードブロックを使い、`-` 行を変更前、`+` 行を変更後として示す
- 差分抜粋はレビュー論点に必要な最小限の hunk に絞る。長い差分全文は貼らない
